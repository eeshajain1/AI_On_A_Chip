...
class CustomConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        out_6bit = F.conv2d(x, weight_6bit, self.conv.bias,
                            stride=self.conv.stride, padding=self.conv.padding,
                            dilation=self.conv.dilation, groups=self.conv.groups
                            )
        return self.percent * out_8bit + (1-self.percent) * out_6bit
...
model_custom = ResNet8().to(device)
model_custom.load_state_dict(torch.load("lab1part2.pth", map_location=device), strict=False)

criterion = nn.CrossEntropyLoss()
optimizer = optim.SGD(model_custom.parameters(), lr=learning_rate, momentum=0.9, weight_decay=5e-4)
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs)

# Train the model with 8-bit
best_acc = 0
for epoch in range(num_epochs):
    train(model_custom, epoch)
    acc = test(model_custom, epoch)
    scheduler.step()
    ...
    if acc > best_acc:
        print(f'Saving model, acc: {acc:.3f} > best_acc: {best_acc:.3f}')
        best_acc = acc
        torch.save(model_custom.state_dict(), 'lab1part2_qat.pth')
...
Conv2dClass = QuantizedConv2d6
model = ResNet8().to(device)
model.load_state_dict(torch.load("lab1part2_qat.pth", map_location=device), strict=False)
# criterion = nn.CrossEntropyLoss()
# optimizer = optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9, weight_decay=5e-4)
# scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs)
#Run inference 
