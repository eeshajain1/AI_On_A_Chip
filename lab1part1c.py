...
class SimpleNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = Conv2dWrapper(3, 32, 3, padding = 1) #first channel must set to 3 for this dataset as it has three color channels
        #think of the above as 32 cubes, each with depth 3 (first parameter) and height and width 3x3 (third parameter)
        self.pool = nn.MaxPool2d(2, 2)
        self.conv2 = Conv2dWrapper(32, 32, 3, padding = 1) #this is 32 filter cubes (2nd parameter) with depth 32 (first parameter) of size 3x3
        self.fc1 = nn.Linear(64 * 4 * 4, 120) #(infeatures, outfeatures) --> y = Wx + b, this just automatically caluclates the size of W
        self.fc2 = nn.Linear(120, 10)
        self.conv3 = Conv2dWrapper(32, 64, 3, padding=1) #third convolutional layer with 32 input channels and 64 output channels 
...
'''first test the network with the weights from part a'''
'''first train the weights that we stored in part a'''
model = SimpleNet().to(device)
model.load_state_dict(torch.load('lab1part1a.pth'), strict=False)
...
print(f'Accuracy of the network on the 10000 test images (DIRECT INFERENCE WITHOUT TRAINING): {100 * correct / total:.2f}%')
...
torch.save(net.state_dict(), 'lab1part1c.pth')